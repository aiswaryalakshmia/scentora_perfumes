    ...
    if request.method == "POST":
        ...
        if len(full_name) == 0:
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Full name is required"},
                status=400,
            )
        ...
        if not re.match(r"^[A-Za-z ]+$", full_name):
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Full name can contain only letters"},
                status=400,
            )
        ...
        if len(full_name) > 150:
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Full name cannot exceed 150 characters"},
                status=400,
            )
        ...
        if len(email) == 0:
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Email is required"},
                status=400,
            )
        ...
        if len(email) > 254:
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Email address is too long"},
                status=400,
            )
        ...
        if not re.match(email_pattern, email):
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Enter a valid email address"},
                status=400,
            )
        ...
        if User.objects.filter(email=email).exists():
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Email already exists"},
                status=400,
            )
        ...
        if len(mobile_number) == 0:
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Mobile number is required"},
                status=400,
            )
        ...
        if not mobile_number.isdigit():
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Mobile number must contain only digits"},
                status=400,
            )
        ...
        if len(mobile_number) != 10:
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Mobile number must be 10 digits"},
                status=400,
            )
        ...
        if User.objects.filter(mobile_number=mobile_number).exists():
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Mobile number already exists"},
                status=400,
            )
        ...
        if mobile_number[0] not in "6789":
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Enter a valid mobile number"},
                status=400,
            )
        ...
        if password_error:
            return render(
                request,
                "signup.html",
                {**base_context, "error": password_error},
                status=400,
            )
        ...
        if referral and len(referral) > 20:
            return render(
                request,
                "signup.html",
                {**base_context, "error": "Referral code is invalid"},
                status=400,
            )
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if not email:
            return render(
                request, "login.html", {"error": "Email is required"}, status=400
            )

        if not password:
            return render(
                request, "login.html", {"error": "Password is required"}, status=400
            )
...
def verify_otp(request):
    ...
    if latest_otp:
        expires_at_timestamp = int(
            latest_otp.expires_at.timestamp() * 1000
...
def reset_password(request):
    ...
    if request.method == "POST":
        ...
        if password_error:
            return render(
                request, "reset_password.html", {"error": password_error}, status=400
            )
