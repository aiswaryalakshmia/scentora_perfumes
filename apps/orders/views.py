...
from apps.userprofile.wallet_utils import (
    credit_wallet,
    debit_wallet,
    get_wallet_balance,
    has_been_refunded,
...
from .models import Coupon, CouponUsage, Order, OrderAddress, OrderItem, Payment
from .utils import (
    calculate_item_refund,
    release_abandoned_razorpay_orders,
    validate_coupon,
)
...
def checkout(request):
    ...
    already_used_ids = CouponUsage.objects.filter(user=request.user).values_list(
        "coupon_id", flat=True
    )
    ...
    available_coupons = (
        ...
        .exclude(used_count__gte=db_models.F("usage_limit"))
    )
...
def place_order(request):
    if request.method == "POST":
        ...
        total_amount = sum(
            item.product_variant.price * item.quantity for item in cart_items
        )
        discount_amount = sum(
        ...
        if payment_method == "wallet":
            ...
            if current_balance < final_amount:
                messages.error(
                    request,
                    f"Insufficient wallet balance. Available: ₹{current_balance}, Required: ₹{final_amount}.",
                )
                return redirect("checkout")
        ...
        try:
            with transaction.atomic():
                ...
                if coupon_code:
                    ...
                    revalidated_coupon, revalidated_discount, coupon_error = (
                        validate_coupon(coupon_code, user, cart_total_for_coupon)
                    )
                    ...
                    if coupon_error:
                        ...
                        request.session.pop("coupon_discount", None)
                        messages.warning(
                            request, f"Your coupon could not be applied: {coupon_error}"
                        )
                    else:
...
def order_management(request):
    ...
    orders = Order.objects.select_related("user", "order_address").order_by(
        "-created_at"
    )
...
def admin_order_detail(request, order_id):
    order = get_object_or_404(
        Order.objects.select_related("user", "order_address"), id=order_id
    )
    ...
    order.display_status = (
        "return_requested" if has_return_requested else order.order_status
    )
    ...
    if order.order_status == "return_requested" or has_return_requested:
        ...
        allowed_status_choices = [
            choice for choice in Order.STATUS_CHOICES if choice[0] in allowed_values
        ]
...
def update_order_status(request, order_id):
    ...
    if request.method == "POST":
        ...
        if order.order_status == "return_requested":
            messages.error(
                request,
                "Please use Approve or Reject buttons to handle return request.",
            )
        ...
        if new_status == "cancelled":
            ...
            for item in order.items.filter(status="active").select_related(
                "product_variant"
            ):
                item.product_variant.stock += item.quantity
                item.product_variant.save()
            ...
            if total_refunded > 0:
                messages.success(
                    request,
                    f"Order cancelled. ₹{total_refunded} refunded to customer's wallet.",
                )
            else:
...
def update_item_status(request, order_id, item_id):
    ...
    if request.method == "POST":
        ...
        if new_status == "cancelled" and item.status != "cancelled":
            ...
            messages.success(
                request,
                "Item cancelled, stock restored, and refund processed if applicable.",
            )
        else:
    ...
    if request.method == "POST":
        ...
        if action == "approve":
            ...
            if order.payment_status == "paid" and not has_been_refunded(order):
                ...
                messages.success(
                    request,
                    f"Return approved. Stock restored and ₹{order.final_amount} refunded to customer's wallet.",
                )
            else:
            ...
            order.save()
            messages.success(
                request, "Return request rejected. Order is back to delivered."
            )
    ...
    if request.method == "POST":
        ...
        if action == "approve":
            ...
            if (
                order.payment_status == "paid"
                and refund_amount > 0
                and not has_item_been_refunded(item)
            ):
                credit_wallet(
                    user=order.user,
                    amount=refund_amount,
                ...
                messages.success(
                    request,
                    f"Item return approved. Stock restored and ₹{refund_amount} refunded to customer's wallet.",
                )
            else:
                messages.success(
                    request, "Item return approved. Stock has been restored."
                )
...
def initiate_payment(request, order_id):
    ...
    payment, _ = Payment.objects.get_or_create(
        order=order, defaults={"amount": order.final_amount}
    )
...
def verify_payment(request):
    ...
    try:
        ...
        return redirect(
            f"{reverse('payment_failure', args=[order.id])}?reason=verification_failed"
        )
...
def payment_failure(request, order_id):
    ...
    if reason == "cancelled" and order.order_status == "pending":
        ...
        if order.coupon:
            # Remove usage record
            CouponUsage.objects.filter(coupon=order.coupon, user=request.user).delete()
...
def apply_coupon(request):
    ...
    try:
        ...
        cart_total = sum(
            get_offer_price(item.product_variant)[0] * item.quantity for item in items
        )
    except Cart.DoesNotExist:
        messages.error(request, "Your cart is empty.")
        return redirect("checkout")
    ...
    messages.success(
        request, f"Coupon '{coupon.coupon_code}' applied! You save ₹{discount}."
    )
    return redirect("checkout")
...
def add_coupon(request):
    if request.method == "POST":
        ...
        if discount_type == "flat" and minimum_price_num <= discount_value_num:
            messages.error(
                request,
                "Minimum order amount must be greater than the flat discount value.",
            )
            return render(request, "admin/add_coupon.html", status=400)
...
def edit_coupon(request, coupon_id):
    ...
    if request.method == "POST":
        ...
        if usage_limit_num < coupon.used_count:
            messages.error(
                request,
                f"Usage limit cannot be less than the number of times already used ({coupon.used_count}).",
            )
            return redirect("edit_coupon", coupon_id=coupon.id, status=400)
...
def toggle_coupon_status(request, coupon_id):
    ...
    messages.success(
        request,
        f"Coupon {'activated' if coupon.status == 'active' else 'deactivated'} successfully.",
    )
    return redirect("coupon_management")
...
def sales_report(request):
    ...
    total_discount = (summary["total_discount"] or Decimal("0")) + (
        summary["total_coupon_discount"] or Decimal("0")
    )
...
def sales_report_pdf(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="sales_report_{start}_{end}.pdf"'
    )
    ...
    title_style = ParagraphStyle(
        "title", fontSize=20, spaceAfter=6, fontName="Helvetica-Bold"
    )
    ...
    detail_table = Table(
        table_data, colWidths=[2 * inch, 1.5 * inch, 2 * inch, 2 * inch, 2 * inch]
    )
...
def sales_report_excel(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="sales_report_{start}_{end}.xlsx"'
    )
    wb.save(response)
    return response
