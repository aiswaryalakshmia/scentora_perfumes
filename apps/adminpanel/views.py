...
def admin_dashboard(request):
    ...
    revenue = (
        all_orders.filter(payment_status="paid").aggregate(total=Sum("final_amount"))[
            "total"
        ]
        or 0
    )
    ...
    best_products = (
        OrderItem.objects.filter(
            order__order_status__in=["delivered", "shipped", "processing"]
        )
...
def user_management(request):
    ...
    return render(
        ...
        {"users": page_obj, "search_query": search_query},
    )
